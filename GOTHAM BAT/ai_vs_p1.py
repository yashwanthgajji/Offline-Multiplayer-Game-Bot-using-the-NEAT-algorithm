    ...
    while run1 or run2:
        ...
        for event in pygame.event.get():
            if event.type==pygame.QUIT:
                ...
                quit()
        pipe_ind=0
        if len(pipes)>1 and bird1.x>pipes[0].x+pipes[0].PIPE_TOP.get_width():
            pipe_ind=1
        bird1.move()
        bird2.move()
        output=net.activate((bird1.y,abs(bird1.y-pipes[pipe_ind].height),abs(bird1.y-pipes[pipe_ind].bottom)))
        if output[0]>0.5:
            bird1.jump()
        keys=pygame.key.get_pressed()
        if keys[pygame.K_SPACE] and run2:
            bird2.jump()
        add_pipe=False
        rem=[]
        for pipe in pipes:
